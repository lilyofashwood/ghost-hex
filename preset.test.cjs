...
test('launcher presets change declared UI controls only; URL secrets and invalid options are ignored',
  { skip: !chromium && 'Install Playwright to run browser preset checks' }, async () => {
    ...
    try {
      ...
      const allowed = new Set(["app.js","style.css","image.png","index.html"]);
      const posts = [], errors = [];
      ...
      for (const [query, panel, selected] of [["?mode=decode&placement=after","decode-panel","after"],["?mode=encode&placement=before","encode-panel","before"],["?mode=decode&placement=before","decode-panel","before"],["?mode=encode&placement=after","encode-panel","after"],["?mode=unknown&placement=unknown","encode-panel","before"],["?mode=__proto__&placement=%F0%9D%93%B5","encode-panel","before"]]) {
        ...
        assert.equal(await page.evaluate(() => document.querySelector('#placement-input').value), selected);
